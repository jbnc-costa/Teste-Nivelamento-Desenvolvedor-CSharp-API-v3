...
namespace Questao5.Infrastructure.Sqlite
{
    public class DatabaseBootstrap : IDatabaseBootstrap
    {
        ...
        public List<Contacorrente> ConsultarContaCorrente(int conta)
        {
            using var connection = new SqliteConnection(databaseConfig.Name);

            var contavalor = connection.Query<Contacorrente>("Select idcontacorrente, numero, nome, ativo from contacorrente where numero = @numero",
                new { numero = conta }).ToList();

            return contavalor;
        }

        public string MovimentarContaCorrente(Movimentacao movimentacao)
        {
            using var connection = new SqliteConnection(databaseConfig.Name);

            var movimento = connection.Query<Movimentacao>("Insert Into movimento (idmovimento, idcontacorrente, datamovimento, tipomovimento, valor) values (@idmovimento, @idcontacorrente, @datamovimento, @tipomovimento, @valor)",
                new { idmovimento = movimentacao.Idmovimento, idcontacorrente = movimentacao.Idcontacorrente, datamovimento = movimentacao.Datamovimento, tipomovimento = movimentacao.Tipomovimento, valor = movimentacao.Valor });

            return $"Movimentação do cliente incluido com sucesso";
        }

        public List<Movimentacao> ConsultarMonvimentacaoContaCorrente(string idcontacorrente)
        {
            using var connection = new SqliteConnection(databaseConfig.Name);
